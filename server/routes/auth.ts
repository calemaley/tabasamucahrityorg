...
router.post("/login", (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ sub: row.id, username: row.username }, SECRET, {
      expiresIn: "7d",
    });
    res.json({ token });
  } catch (err) {
...
router.post("/reset-default", (req, res) => {
  ...
  const allowInsecure =
    process.env.ALLOW_INSECURE_ADMIN_RESET === "1" ||
    process.env.NODE_ENV !== "production";
  if (
    REGISTER_SECRET &&
    REGISTER_SECRET !== "" &&
    REGISTER_SECRET === admin_secret
  ) {
    // allowed
  ...
  const DEFAULT_PASS =
    process.env.DEFAULT_ADMIN_PASS || "teamtabasamu908Q@2025";
  try {
    ...
    if (existing) {
      sqlite
        .prepare(
          "UPDATE admins SET password_hash = ?, created_at = ? WHERE id = ?",
        )
      ...
      sqlite
        ...
        .run(id, DEFAULT_ADMIN, hash, Date.now());
    }
...
router.post("/register", (req, res) => {
  ...
  if (REGISTER_SECRET && REGISTER_SECRET !== admin_secret)
    return res.status(403).json({ error: "forbidden" });
  if (!username || !password)
    return res.status(400).json({ error: "username and password required" });
  try {
